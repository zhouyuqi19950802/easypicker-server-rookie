...
export default class FileService {
  ...
  async downloadOne(fileId: number) {
    ...
    if (!isExist) {
      this.behaviorService.add('file', `下载文件失败 用户:${logAccount} 文件:${file.name} 已从本地移除`, {
        account: logAccount,
        name: file.name,
      })
  ...
  async batchDownload(ids: number[], zipName: string) {
    ...
    if (keys.length === 0) {
      this.behaviorService.add('file', `批量下载文件失败 用户:${logAccount} 文件均已从本地移除`, {
        account: logAccount,
      })
      throw publicError.file.notExist
    }
